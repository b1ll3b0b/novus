...
// Mirrors Biofactory's pattern for Create's potion/tea/honey fluids. Each fluid's per-mB
// tribute is derived from the corresponding drinkable item's Tribute, scaled to a 250 mB
// tankard — identical economics to dropping the keg/bottle into the cradle.
//
// Why KubeJS (the CrT port is .disabled): biomancy 1.20.1 ships zero @ZenRegister
// bindings, so ZenScript can't import com.github.elenterius.biomancy.api.tribute.* at
// all. KubeJS-Rhino reaches raw JVM via Java.loadClass.
//
// CRITICAL TIMING (v2 fix): all Java.loadClass calls MUST be deferred into postInit,
// NOT at script top level. Top-level loadClass runs during KubeJS' construction phase
...
// an unpopulated CREATOR_MIX item, throwing ExceptionInInitializerError. The class is
// then permanently INITIALIZATION_FAILED in this JVM, biofactory's BiomancyIntegration
// can't initialize, and the script-load exception escapes constructMod — which makes
// Forge skip FMLClientSetupEvent for ALL mods, taking down anything that initializes
// its singleton there (notably SodiumDynamicLights → NPE on world load). One bug,
// game-wide cascade. Outer try/catch is the second line of defense.
//
// Two Rhino traps inside the postInit body:
//   1. FluidTributes.register has 3 overloads (Fluid / RegistryObject / ResourceLocation)
//      with the same second arg → Rhino's overload picker can be ambiguous. We bypass it
//      by naming the exact JVM signature with bracket-syntax method access.
    ...
    try {
        ...
        DRINKS.forEach(id => {
            try {
                ...
                // STARTUP-TIME FALLBACK: precompute via the v3.2b path (Tributes.getTribute →
                // FluidTribute.of). For our drinks this is biomancy's WRONG_ITEM constant
                // (uniform junk values), but it's NON-EMPTY which is all we need to make
                // registration stick so the cradle accepts the fluid. This is the no-regress
                // floor — even if the runtime tag-aware build below fails or finds nothing,
                // we land here.
                ...
                // RUNTIME-LAZY TAG-AWARE TRIBUTE: the real win. addEffect queries
                // ModMobEffectTags.isCradleXxxSource(effect), which requires the user's
                // overlay tags to be bound — and they're loaded at world-start via
                // KubeJS' datapack, AFTER our postInit fires. So we build the real
                // tribute at FIRST call inside the SAM lambda (cradle-tick time, tags
                // are bound by then), cache it, and use the cache for all subsequent
                // ticks. Any failure inside this lambda falls back to the WRONG_ITEM
                // fallback so the cradle never sees null and never crashes its tick.

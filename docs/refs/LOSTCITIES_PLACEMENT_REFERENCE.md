...
The complete tuned `novus` profile. **Client: OFF by default, player-selectable; server: forced on (final, updated 2026-06-16).** `novus` is *not* force-applied to the overworld on the client: `config/lostcities/common.toml` carries no overworld mapping, and there is no `defaultconfigs/lostcities-server.toml` (removed 2026-06-16). Fresh singleplayer worlds are vanilla overworld; players opt into `novus` via the LC world-creation menu (cities overlaid on the unchanged vanilla biome/terrain layout). The **dedicated server forces it on** — the `_server_deploy/` overlay's `config/lostcities/common.toml` carries `dimensionsWithProfiles` including `minecraft:overworld=novus` (no opt-out), applied to the `--release` server zip by `build.sh`. See the `DECISIONS.md` resolution-order entry for why the client/server split is the only way to get both "vanilla default in SP" and "guaranteed on the server." (Supersedes the original "inert / disabled by default" stance and an interim "default-on via `selectedProfile`/`defaultconfigs`" attempt.) The separate `lostcities:lostcity` dimension is mapped to `biosphere` (LC's default for it) on both client and server.
